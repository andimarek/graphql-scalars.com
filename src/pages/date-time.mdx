...
These are valid Strings accepted for the input of Literal Coercion and valid as output of 
Result Coercion:
...
These are invalid Strings which should be rejected as Liternal Coercion Input and should not 
...
If this name is not available one alternatives is `OffsetDateTime` which describes the Scalar quite 
accurately.
